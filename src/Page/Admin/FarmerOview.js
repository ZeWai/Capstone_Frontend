...
export const FarmerOview = () => {
    ...
    const submitNewPlace = () => {
        ...
        axios.delete(`${process.env.REACT_APP_API_SERVER}/api/resetPlace`, { data: newPlaceForm })
            .then((res) => {
                setModal(false);
                //reload farmer list
                dispatch(GetClientListThunk());
    ...
    return (
        <>
            ...
            {ClientListByFarmer && ClientListByFarmer[0] !== undefined ? ClientListByFarmer.map((client, index) =>
                <div key={client.id}>
                    <div className="admin-client-overview-wrapper">
                        <div className="admin-list row" style={index % 2 === 0 ? { backgroundColor: "#FFFFFF" } : { backgroundColor: "#f5f5f5" }}>
                            ...
                            <div className="admin-list-farmer-edit-wrapper col-3">
                                <button style={index % 2 === 0 ? { backgroundColor: "#FFFFFF" } : { backgroundColor: "#f5f5f5" }} className="admin-list-farmer-edit-btn" onClick={editList} id={index}>
            ...
            <Modal className="admin-popup-wrapper" show={modal}>
                ...
                <Modal.Footer className="admin-popup-footer">
                    <Button className='admin-popup-submit-btn' onClick={submitNewPlace} name={CurrentFarmer} >Submit</Button>
                    <Button className='admin-popup-cancel-btn' onClick={() => setModal(false)} name={CurrentFarmer}>Cancel</Button>
                </Modal.Footer>
            </Modal >
        </>
    )
